...
var positionParts = function (turnAxis, level, direction) {
  ...
  $.each(elements, function (i, el) {
    $.each(axis, function (j, ax) {
      $(el).attr("data-" + ax, data[turnAxis][oppositeDirection][level][i][ax]);
    });
  });
};
/**
...
var performTurn = function (rotate3d, turnAxis, direction, callback) {
  ...
  $(".grip")
    ...
    .css({
      ...
      "transition-duration": turnDuration + "ms",
    })
...
var moveSwiper = function (e) {
  if (canvas.swiper && canvas.swiper.play) {
    ...
    canvas.swiper.distance.x =
      ((canvas.swiper.start.x - canvas.swiper.move.x) / canvas.swiper.size.x) * 90;
    canvas.swiper.distance.y =
      ((canvas.swiper.start.y - canvas.swiper.move.y) / canvas.swiper.size.x) * 90;
    ...
    if (canvas.swiper.axis) {
      ...
      var factor = 1;
      factor = canvas.swiper.side === "x" && canvas.swiper.control === "front" ? -1 : factor;
      factor = canvas.swiper.side === "y" && canvas.swiper.control === "right" ? -1 : factor;
      factor = canvas.swiper.side === "x" && canvas.swiper.control === "bottom" ? -1 : factor;
      canvas.swiper.degree =
        ((canvas.swiper.start[canvas.swiper.turn] - canvas.swiper.move[canvas.swiper.turn]) /
          canvas.swiper.size[canvas.swiper.side]) *
        90 *
        factor;
      $gripAxis.css({
        transform: "rotate3d(" + rotate3d[canvas.swiper.axis] + canvas.swiper.degree + "deg)",
      });
    }
  }
};
...
var moveCube = function (e) {
  ...
  var transformation = "";
  if ((x > canvas.threshold || x < -1 * canvas.threshold) && canvas["axis"] !== "x") {
    canvas["axis"] = "y";
    ...
    canvas.moved = true;
  } else if ((y > canvas.threshold || y < -1 * canvas.threshold) && canvas["axis"] !== "y") {
    canvas["axis"] = "x";
...
var releaseCube = function (e) {
  if (canvas.down) {
    ...
    canvas["mouse"]["bouce"] = (canvas["mouse"]["distance"] / canvas["mouse"]["time"]) * 100;
...
$(function () {
  ...
  $(document).on("mousedown", onMouseDown).on("mouseup", releaseCube).on("mousemove", moveCube);
